...
const ReservationDefaultContent = () => {
  ...
  const handleDateChange = async () => {
    try {
      ...
      if (!token) throw new Error('Authentication token not found');
      console.log('person_uid:', reservationData.provider?.[0]?.user?.id);

      // Build the payload using the required keys and mapping from reservationData
      const payload = {
        _method: 'put',
        sp_uid: reservationData.sp_id,
        person_uid: reservationData.provider?.[0]?.user?.id, // <-- FIXED
        offer_uid: reservationData.offer_uid || '',
        booking_status: reservationData.status_num,
      ...
      if (res.data?.success) {
        enqueueSnackbar('Arrival date updated successfully!', { variant: 'success' });
        setIsDateModalOpen(false);
        fetchReservation(); // Refresh reservation data
      } else {
        enqueueSnackbar(res.data?.message || 'Failed to update arrival date', { variant: 'error' });
      }
    } catch (error) {
      console.error('Error updating arrival date:', error);
      enqueueSnackbar(error.response?.data?.message || 'An error occurred while updating arrival date.', { variant: 'error' });
    }
  };
    ...
    <>
      ...
      <div className="  card shadow-sm">
        <div className="card-header">
          <div className="flex flex-col  ">
            ...
            <span className={`badge badge-outline ${reservationData.status === 'Active' ? 'badge-success' : reservationData.status === 'inactive' ? 'badge-danger' : reservationData.status === 'Waiting Payment' ? 'badge-primary' : reservationData.status === 'Waiting Confirmation' ? 'badge-warning' : 'badge-secondary'}`}>{reservationData.status}</span>
        </div>
        {/* Status badge */}
      ...
      <div className="card shadow-sm">
        ...
        <div className="card-body">
          <div className="card p-4">
            ...
            <button 
              className={`btn btn-outline ${reservationData.status === 'Waiting Confirmation' ? "btn-warning" : "btn-success"} flex justify-center items-center mt-4 border`}
              onClick={() => reservationData.status === 'Waiting Confirmation' && setIsDateModalOpen(true)}
            > 
              {reservationData.status === 'Waiting Confirmation' ? "Edit Arrival Date" : "Confirmed"}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="card mt-6 shadow-sm">
        <div className="card-header">
          ...
          <h3 className='card-title'> {reservationData.coupons[0].is_used}/{reservationData.num_coupons} </h3>
        </div>
        <div className="card-body">
          <div className="flex flex-col gap-2">
            <div className="grid grid-cols-1 gap-2">
              {reservationData.coupons && reservationData.coupons.length > 0 && reservationData.status === 'Active' ? (
      ...
      <Modal open={isDateModalOpen} onClose={() => setIsDateModalOpen(false) }>
        <div className="card w-1/2 h-1/2 items-center justify-center gap-4 relative " style={{ margin: 'auto', maxWidth: '500px' }}>
          <div className="card-header w-full">
            ...
            <button onClick={() => setIsDateModalOpen(false)}><KeenIcon icon='cross-circle' className='text-xl'/></button>
          </div>
          ...
          <div className="card-footer justify-end w-full gap-4">
            ...
            <button className="btn btn-outline btn-primary" onClick={handleDateChange}>Update Date</button>
          </div>
        </div>
      </Modal>
    </>
