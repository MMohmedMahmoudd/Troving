...
const EditPaymentMethodPage = () => {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        ...
        // Fetch English data
        const responseEn = await axios.get(`${import.meta.env.VITE_APP_API_URL}/payment-methods?filter[payment_id]=${id}`, {
          headers: {
        ...
        // Fetch Arabic data
        const responseAr = await axios.get(`${import.meta.env.VITE_APP_API_URL}/payment-methods?filter[payment_id]=${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
            'Accept-Language': 'ar'
          }
  ...
  {loading ? (
    ...
    <EditPaymentMethodContent categoryData={categoryData} />
  ) : (
