...
const CountrySelect = ({ formik }) => {
  ...
  const handleChange = (selectedOption) => {
    formik.setFieldValue('country_id', selectedOption ? selectedOption.value : '');
  };
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <Select
        ...
        isLoading={loading}
        options={options}
        value={options.find(opt => opt.value === +formik.values.country_id) || null}
      ...
      />
        ...
        <p className="text-red-500 text-xs mt-1">{formik.errors.country_id}</p>
      )}
    </div>
  );
};
