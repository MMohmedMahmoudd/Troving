...
const ReservationTap = ({ providerId }) => {
  ...
  const columns = useMemo(() => [
      ...
      cell: ({ row }) => {
        ...
        return (
          ...
          <div className="flex items-center gap-3">
            <img
              src={person_image || '/media/avatars/blank.png'}
              alt={person_name}
...

    {
      ...
      cell: ({ row }) => (
          <span className={`badge flex-nowrap badge-${row.original.status_name === 'Waiting Payment' ? 'warning' : row.original.status_name === 'confirmed' ? 'success' : row.original.status_name === 'cancelled' ? 'danger' : row.original.status_name === 'expired' ? 'danger' : row.original.status_name === 'completed' ? 'success' : 'danger'} badge-outline capitalize`}>
