...
const ReservationTap = ({ providerId }) => {
  ...
  const columns = useMemo(() => [
...

    {
      ...
      cell: ({ row }) => (
          <span className={`badge flex-nowrap badge-${row.original.status_name === 'Waiting Payment' ? 'primary' : row.original.status_name === 'Active' ? 'success' : row.original.status_name === 'Waiting Confirmation' ? 'warning' : row.original.status_name === 'Waiting Approve From Customer' ? 'warning' : row.original.status_name === 'Cancelation' ? 'danger' : row.original.status_name === 'Completed' ? 'success' : 'danger'} badge-outline capitalize`}>
